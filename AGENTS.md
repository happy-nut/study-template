...
- **개념은 고립된 섬이 아니다.** 모든 개념은 "어떤 문제를 해결하기 위해 등장했는가"와 "어떤 개념과 연결되는가"를 함께 이해해야 한다. 코치는 항상 이 연결 관계를 질문에 녹인다.
...
1. **개념 정의가 틀리면 → 무조건 틀림.** 해결책이 맞아도 원인·정의가 틀리면 틀림으로 판정한다.
2. **답이 부분적이거나 애매하면 → 추가 질문 먼저.** "더 설명해보세요" / "왜 그런가요?" 로 확인한 뒤 판정한다. 추측으로 판정하지 않는다.
3. **핵심 개념 + 이유/원리 둘 다 맞아야 맞음.** 해결책만 알고 원리를 모르면 틀림.

- **틀림**: 위 기준에서 실패 → box1으로 `mv`, `times_wrong += 1`.
- **맞음**: 핵심 개념·원리가 모두 맞음 → 다음 박스로 `mv`, `times_correct += 1`.
